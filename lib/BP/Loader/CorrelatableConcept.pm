...
sub new($@) {
	...
	# Command line agent detection (pigz vs gzip) is done in BP::Loader::Agents
...
sub __SortCompressed(\@\@) {
	...
	# my $cmd = "('$GREP' '^#' '$infile' ; '$GREP' -v '^#' '$infile' | head -n 1 ; '$GREP' -v '^#' '$infile' | tail -n +2 | '".BP::Loader::Agents::SORT."' --parallel=$NUMCPUS -S 50% $sortkeys ) | '".BP::Loader::Agents::GZIP."' -9c > '$tmpoutfilename'";
	
	my $inpipeStr = __inpipe2Str(@{$inpipe});
	
	my $cmd = "$inpipeStr | '".BP::Loader::Agents::SORT."' --parallel=$NUMCPUS -S 50% $sortkeys | '".BP::Loader::Agents::GZIP."' -9c > '$tmpoutfilename'";
	...
	unless(exists($self->{__prepared})) {
		...
		if(defined($referenceConceptFile)) {
			if(defined($self->{__compressed})) {
				...
				if(open(my $H,'-|',BP::Loader::Agents::GUNZIP,'-c',$sortedFilename)) {
					$self->{H} = $H;
				} else {
